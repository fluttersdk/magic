...
- **`MagicFile.size` can now be null for a picked file.** The type did not change, only what reaches it: v11's `PlatformFile.size` was non-nullable, v12 replaces it with `lengthSync()`, which reports the size the native picker already knew and null when it knew none. A `content://` or `blob:` pick is where that shows up. `readAsBytes().length` is the fallback, at the cost of reading the file. Code that wrote `file.size!` or fed the value straight into arithmetic needs a null check. (`lib/src/facades/pick.dart`)
