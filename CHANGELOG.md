...
- **`ReverbBroadcastDriver` reported `reconnecting` after a socket drop even with `reconnect: false`, so a consumer of `connectionState` was told a reconnect was coming when `_scheduleReconnect` had already returned without arming one.** `_onDone` and `_onError` now report `disconnected` instead when the `reconnect` config key is off, and still report `reconnecting` when it is on. `AuthChannelSubscription` also treated a `connecting` state as "a reconnect is pending" and skipped its own `Echo.connect()`, which stranded the subscription if that own connect call had failed at boot (no reconnect scheduled, no further state emitted, the marker stuck on `connecting` forever); the gate now only recognizes `reconnecting`, and a failed own `connect()` resets the tracked state to `null` before rethrowing so the next `sync()` retries. (`lib/src/broadcasting/drivers/reverb_broadcast_driver.dart`, `lib/src/broadcasting/auth_channel_subscription.dart`, `doc/digging-deeper/broadcasting.md`, `skills/magic-framework/`)
