...
- **`MagicVaultService` now configures `mOptions` for macOS, so an unsigned or ad-hoc local build can store a secret.** The constructor previously passed only `aOptions` and `iOptions`, leaving macOS on `flutter_secure_storage`'s default `usesDataProtectionKeychain: true`, which needs a `keychain-access-groups` entitlement and therefore a signing identity: every write failed with `PlatformException(-34018, errSecMissingEntitlement)` on a build with no certificate installed. `usesDataProtectionKeychain: false` does not set the flag false, it omits the key from the query entirely (`flutter_secure_storage_darwin` `FlutterSecureStorage.swift:227-231`), so the item lands in the legacy login keychain instead, which needs no entitlement. There is no migration between the two keychains: a consumer that stores a secret unsigned, then gains a signing identity and rebuilds, reads from the data protection keychain and finds nothing, because the old item is still sitting in the legacy one under the same key. `first_unlock_this_device` keeps the item device-bound rather than the default `first_unlock`, matching a credential that should not roam via iCloud Keychain sync. The iOS and Android options are unchanged. (`lib/src/security/magic_vault_service.dart`)

### Added

- **`FakeVaultService.throwOnGet` and `.throwOnPut`, so a consumer can test its own vault-failure branch.** Every prior override was a no-throw body over an in-memory map, so nothing exercised the path a real keychain failure takes through `MagicVaultService.get`/`.put`, which wrap a `PlatformException` as `MagicVaultException`. Both default to a `MagicVaultException` and accept a custom error; both are cleared by `reset()`. Each only affects its own operation: a throw armed on `get` does not touch `put`, and vice versa. (`lib/src/testing/fake_vault_service.dart`)
