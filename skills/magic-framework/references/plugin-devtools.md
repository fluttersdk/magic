<!-- magic_devtools v0.0.5 | Updated: 2026-09-21 -->
...
Those two floors are the ones `magic_devtools` itself declares from 0.0.5, beside `magic ^0.0.15` and `fluttersdk_wind ^1.6.2`: the newest of each sibling at that release. The perf data path alone needs less, and those minimums are why the floors matter at all: `perf_readers.dart` (dusk 0.0.12), `FramePerfWatcher` / `TelescopeStore.recentFramePerf` (telescope 0.0.5), `MagicController.onRefreshUI` (magic 0.0.7) and `WindPerfCounters` (wind 1.5.0). A caret range resolves to the newest, so a fresh graph always worked; an app whose own constraints hold one sibling back gets a satisfiable graph that then fails on undefined symbols.
