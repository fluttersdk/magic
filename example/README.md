...
In-repo smoke reference for the `magic` framework. Deliberately minimal: 17 dart files, zero plugin dependencies, pinned to a local path `magic: {path: ..}`, runs in magic's CI on every push, and regenerated per release. This is what pub.dev shows on the `magic` package example tab.
